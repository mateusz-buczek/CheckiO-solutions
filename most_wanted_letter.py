...
def checkio(text): # clean and step by step
    result = ''
    counter = 0
    text = text.lower() # making sure all characters ale lower-cased 
    for el in text: # looping through string and checking letters only 
        if el.isalpha():
            if text.count(el) > counter: #comparing occurrences 
                counter = text.count(el)
                result = str(el)
            elif text.count(el) == counter and str(el) < result: #comparing equal occurrences and precendence 
                counter = text.count(el)
                result = str(el)
            else:
                continue
