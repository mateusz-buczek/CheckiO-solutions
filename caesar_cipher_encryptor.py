...
def to_encrypt(text, delta):
    ...
    result = '' # variable returnig the rusult
    for el in text:

        if el.isalpha(): # checking if element is a letter
            el = ord(el) + delta # switching to unicode
            ...
            result += chr(el) # switching unicode to character and adding to result

        else:
            result += el # adding to result without change for non-alphabetical characters
